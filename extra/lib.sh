  ...
  if [[ $__docker = true ]]; then
    cat <<- EOF > /root/tmp/certbot.sh
		...
		if [[ ! ( -d /etc/letsencrypt && "\$(ls -A /etc/letsencrypt)" ) ]]; then
		    /usr/bin/certbot-auto certonly -n --agree-tos --standalone --standalone-supported-challenges http-01 -m "$__myemail" -d "$__mydomain"
		fi
...
EOF
    ...
    /usr/bin/certbot-auto certonly -n --agree-tos --standalone --standalone-supported-challenges http-01 --register-unsafely-without-email -d "$__mydomain"
